...
function animate() {
    ...
    TweenMax.to(L, a, {left: M, delay: d}), TweenMax.to(_, t, {left: $, delay: f}), TweenMax.to(H, e, {
        left: x,
        delay: p
    }), TweenMax.to(N, n, {left: I, delay: m}), TweenMax.to(C, o, {left: T, delay: u}), TweenMax.to(S, r, {
        left: B,
        delay: y
    }), TweenMax.to(q, l, {left: E, delay: h}), TweenMax.to(G, i, {left: k, delay: v}), TweenMax.to(Y, s, {
        left: b,
        delay: g
...
function countdown(sec, prefix = '') {
    ...
    if (sec < 0) {
        $(".countdownnum").html(prefix + " 00:00");
    } else {
        $(".countdownnum").html(`${prefix} ${showminute}:${showsec}`);
    }

}

function countdowntimer(timer, isOpen = true, callback) {
    if (!isOpen) {
        return;
    }
    if (timer >= 0) {
...
function startanimation() {
    ...
    playSound('running');
    audioStatus = 'run';
    $(".wheel10a img").css("animation", "twinkling 0.01s  ease-in-out  infinite"), playing = document.getElementById("running"), ifopen && isplay && playing.play(), addint = setInterval(function () {
        ShipUndulation()
    }, 200), wheelon();
...
function finishgame(finalresult, callback) {
    setTimeout(function () {
        // 暂停车辆启动声音
        pauseSound('running');
        if (ifopen) {
    ...
    $(".flashlight").delay(3200).fadeIn(100, function () {
        ...
        waitfinish = false;
        resultpage(finalresult, callback);
    });
}

// 结束
function resultpage(finalresult, callback) {
    $(".page1").css("display", "block");
    $(".page2").css("display", "none");
    if (waitfinish == true) {
        return false;
    ...
    TweenLite.delayedCall(2, winnerpage, [finalresult, callback]);

}

function showcurrentresult(a) {
    for (var t = a.split(","), e = 0; e < 10; e++) {
        var n = "#pos" + (e + 1), o = -64 * (t[e] - 1);
        $(n).css("background-position", "0px " + o + "px")
    }
}

function winnerpage(a, callback) {
    ...
    // 复位
    TweenLite.delayedCall(2.5, function (results, callback, postMessageToTop) {
        startcountdown();
        postMessageToTop('resetCallback', results);
        if (typeof callback === 'function') {
            callback(results);
        }
    }, [a, callback, postMessageToTop]);
}
...
var animation1, animation2, waitfinish, addint, carHeight = [], isplay = !0, ifopen = !1, playing = "";
$(function () {
...
fun.stateSound = function () {
    running.play(), kaisound.play(), cuttime.play(), setTimeout(function () {
        running.pause(), kaisound.pause(), cuttime.pause()
    }, 1)
